    ...
    binaries {
        all {
            buildArgs.addAll(
                ...
                "-O3",
                // Native builds can only work with around half as many recursive calls as OpenJDK JVM calls, so
                // we pass this in as a build-time system property to establish a lower limit than is deployed
                // with a JVM-run build.
                "-Dname.djsweet.thorium.compiletime.kvp.safetyfactor=25"
            )
        }
